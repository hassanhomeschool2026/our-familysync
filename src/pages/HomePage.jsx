...
function getHeaderTheme(hour, weatherCode) {
  ...
  if (hour >= 22 || hour < 6) {
    return {
      bg: isCloudy
        ? 'linear-gradient(135deg, #1a237e 0%, #37474f 55%, #263238 100%)'
  ...
  if (hour >= 6 && hour < 12) {
    return {
      bg: isCloudy
        ? 'linear-gradient(135deg, #b0bec5 0%, #cfd8dc 55%, #eceff1 100%)'
        : isClear
  ...
  return {
    bg: isCloudy
      ? 'linear-gradient(135deg, #8d8fa8 0%, #6d597a 55%, #4a5568 100%)'
...
export default function HomePage() {
  ...
  return (
    <div className="space-y-4 pb-4">
      <div className="rounded-2xl p-5 relative overflow-hidden" style={{ background: theme.bg }}>
        ...
        <div className="flex items-start justify-between relative z-10">
          ...
          {weather && (
            <a
              ...
              className="bg-white/15 backdrop-blur-sm rounded-xl px-3 py-2 text-right ml-3 flex-shrink-0 cursor-pointer hover:bg-white/25 transition-colors no-underline"
            >
              <div className={`text-xs mb-0.5 ${theme.subColor}`}>
              ...
              {cityName ? <div className={`text-[10px] ${theme.subColor}`}>{cityName}</div> : null}
            </a>
          )}
        </div>
      </div>
              ...
              return (
                <div key={m.id} className="flex flex-col items-center gap-1 min-w-[60px]">
                  <div className="relative">
                    ...
                    />
                    <div
                      className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-card ${checkin ? 'bg-green-500' : 'bg-muted-foreground/30'}`}
                    />
                  </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-card border border-border rounded-xl p-3">
          ...
          <button
            ...
            className="text-[10px] text-primary mt-2 flex items-center gap-0.5"
          >
      ...
      <div className="bg-card border border-border rounded-xl p-4">
        ...
        <button
          ...
          className="text-xs text-primary mt-2 flex items-center gap-0.5"
        >
